...
import { deleteWaitStaffAssignmentById } from 'apiSdk/wait-staff-assignments';
...
function TableViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<TableInterface>(
    ...
    () =>
      getTableById(id, {
        relations: ['restaurant', 'order', 'wait_staff_assignment'],
      }),
  );
  ...
  const wait_staff_assignmentHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('wait_staff_assignment', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Wait Staff Assignments
                    </Text>
                    <NextLink passHref href={`/wait-staff-assignments/create?table_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.wait_staff_assignment?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/wait-staff-assignments/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>
                              {hasAccess(
                                'wait_staff_assignment',
                                ...
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/wait-staff-assignments/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                              ...
                              {hasAccess(
                                ...
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    wait_staff_assignmentHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
